...
SHORTCUT_IDENTIFIER_COLS = [
    "srcip", "dstip", "sport", "dsport", "flow_id", "timestamp", "id", "stime", "ltime"
]
...
def resolve_default_flow_csv_paths() -> List[Path]:
    """Locates complete UNSW-NB15 flow CSV files: UNSW-NB15_1.csv, UNSW-NB15_2.csv, UNSW-NB15_3.csv, UNSW-NB15_4.csv."""
    repo_root = Path(__file__).resolve().parent.parent
    csv_dir = repo_root / "data" / "UNSW-NB15 Dataset" / "CSV Files"
    target_names = ["UNSW-NB15_1.csv", "UNSW-NB15_2.csv", "UNSW-NB15_3.csv", "UNSW-NB15_4.csv"]
    paths = [csv_dir / name for name in target_names if (csv_dir / name).exists()]
    if not paths:
        # Fallback to any UNSW-NB15 data CSVs
        alt = list(csv_dir.glob("UNSW-NB15_*.csv")) if csv_dir.exists() else []
        paths = sorted([p for p in alt if "GT" not in p.name and "features" not in p.name and "LIST" not in p.name])
    return paths


def resolve_default_pcap_paths() -> List[Path]:
    """Locates target PCAP capture slices from 22-1-15 and 17-2-15."""
    repo_root = Path(__file__).resolve().parent.parent
    pcap_root = repo_root / "data" / "UNSW-NB15 Dataset" / "pcap files"
    slices = []
    # 22-1-15 captures (1.pcap .. 5.pcap)
    d1 = pcap_root / "22-1-15"
    if d1.exists():
        for i in range(1, 6):
            p = d1 / f"{i}.pcap"
            if p.exists():
                slices.append(p)
    # 17-2-15 captures (1.pcap .. 5.pcap)
    d2 = pcap_root / "17-2-15"
    if d2.exists():
        for i in range(1, 6):
            p = d2 / f"{i}.pcap"
            if p.exists():
...
class UNSWPreprocessor:
    ...
    def ingest_flow_telemetry(
        self,
        csv_paths: List[Union[str, Path]],
    ) -> Tuple[Dict[int, np.ndarray], Dict[int, int], Dict[int, int]]:
        """Ingests heterogeneous flow CSVs lazily via Polars, aggregating into continuous Unix epoch 0.5s bins.

        Enforces zero data leakage: strips shortcut identifiers, handles hex port notation,
        and computes macro flow metrics.

        Returns:
            Tuple of:
              - flow_features: Dict[bin_id, np.ndarray(16,)]
              - flow_labels: Dict[bin_id, int]
              - flow_mitre: Dict[bin_id, int]
        """
        valid_paths = [Path(p) for p in csv_paths if Path(p).exists()]
        if not valid_paths:
            logger.warning("No valid flow CSV paths provided to ingest_flow_telemetry.")
            return {}, {}, {}

        logger.info(f"Ingesting {len(valid_paths)} flow CSV files lazily via Polars...")
        sanitized_lfs = []
        for p in valid_paths:
            raw_lf = self.scan_unsw_csv(p)
            cleaned_lf = self.clean_and_sanitize(raw_lf)
            sanitized_lfs.append(cleaned_lf)

        combined_lf = pl.concat(sanitized_lfs) if len(sanitized_lfs) > 1 else sanitized_lfs[0]
        bin_dt = self.bin_duration_sec
        ...
        binned_df = (
            combined_lf
            .with_columns([
                (pl.col("timestamp") / bin_dt).floor().cast(pl.Int64).alias("bin_id")
            ])
            .group_by("bin_id")
            .agg(agg_exprs)
            .sort("bin_id")
        ...
        for i, b_id in enumerate(bin_ids):
            flow_features[int(b_id)] = feat_matrix[i]
            flow_labels[int(b_id)] = int(label_vec[i])
            flow_mitre[int(b_id)] = int(mitre_vec[i])

        logger.info(
            f"Extracted {len(flow_features)} flow temporal bins "
            f"(Attacks: {sum(flow_labels.values())}, Benign: {len(flow_labels) - sum(flow_labels.values())})"
        )
        return flow_features, flow_labels, flow_mitre

    def ingest_packet_telemetry(
        self,
        pcap_paths: List[Union[str, Path]],
        max_packets_per_slice: int = 250_000,
        chunk_size: int = 8 * 1024 * 1024,
    ) -> Dict[int, np.ndarray]:
        """Streams target PCAP slices using FastPCAPParser into continuous Unix epoch 0.5s bins.

        Extracts:
          - Mean TTL and TTL variance
          - Mean TCP window size and window variance
          - IP fragmentation ratio
          - Mean payload length and payload entropy
          - SYN and RST packet rates per second
        """
        valid_paths = [Path(p) for p in pcap_paths if Path(p).exists()]
        if not valid_paths:
            logger.warning("No valid PCAP paths provided to ingest_packet_telemetry.")
            return {}

        logger.info(f"Ingesting {len(valid_paths)} target PCAP slices via FastPCAPParser streaming...")
        pcap_parser = FastPCAPParser(bin_duration_sec=self.bin_duration_sec)
        epoch_bins, meta = pcap_parser.stream_slices_to_epoch_bins(
            pcap_paths=valid_paths,
            max_packets_per_slice=max_packets_per_slice,
            chunk_size=chunk_size,
        )
        logger.info(
            f"Ingested {meta['total_packets']:,} packets across {len(valid_paths)} slices "
            f"({meta['total_bytes_mb']:.1f} MB, {meta['throughput_mb_s']:.1f} MB/s) -> {len(epoch_bins)} bins."
        )
        return epoch_bins
    ...
    def fuse_modalities(
        ...
        """Fuses flow and packet modalities synchronized along continuous Unix epoch timeline.

        If a bin has both flow and packet data, features are deterministically combined.
        If a bin has only flow or only packet data, applies deterministic fallback
        without synthesizing fake ground-truth noise.

        Returns:
            Tuple of (features, labels, mitre_stages, timestamps)
        """
        ...
        for i, b_id in enumerate(all_bin_ids):
            timestamps[i] = b_id * self.bin_duration_sec
            ...
            if has_flow and has_packet:
                ...
                # Deterministic fallback without synthesizing fake ground-truth noise
        ...
        logger.info(
            f"Modality Fusion complete: {n_bins} total bins "
            f"(Synchronized Dual-Stream: {num_fused}, Flow-Only: {num_flow_only}, PCAP-Only: {num_pcap_only})"
        )
    ...
    def chronological_split(
        ...
        If timestamps are provided and span multiple capture days (e.g. UNSW-NB15 22-1-15 and 17-2-15),
        partitions chronologically within each capture day to avoid chronological distribution skew
        while strictly preserving temporal ordering within each experiment session.
        """
        total_bins = len(features)
        if total_bins == 0:
            return features, labels, mitre_stages, features, labels, mitre_stages

        if timestamps is not None and len(timestamps) == total_bins:
            day1_mask = timestamps < 1.423e9
            day2_mask = timestamps >= 1.423e9

            if np.any(day1_mask) and np.any(day2_mask):
                logger.info("Multi-day experiment detected (22-01-2015 & 17-02-2015). Partitioning chronologically per capture day...")
                d1_f, d1_l, d1_m = features[day1_mask], labels[day1_mask], mitre_stages[day1_mask]
                d1_split = int(np.floor(self.train_ratio * len(d1_f)))

                d2_f, d2_l, d2_m = features[day2_mask], labels[day2_mask], mitre_stages[day2_mask]
                d2_split = int(np.floor(self.train_ratio * len(d2_f)))

                train_feats = np.concatenate([d1_f[:d1_split], d2_f[:d2_split]])
                train_labels = np.concatenate([d1_l[:d1_split], d2_l[:d2_split]])
                train_mitre = np.concatenate([d1_m[:d1_split], d2_m[:d2_split]])

                val_feats = np.concatenate([d1_f[d1_split:], d2_f[d2_split:]])
                val_labels = np.concatenate([d1_l[d1_split:], d2_l[d2_split:]])
                val_mitre = np.concatenate([d1_m[d1_split:], d2_m[d2_split:]])

                logger.info(
                    f"Per-day chronological split (Train {self.train_ratio*100:.0f}% / Val {(1-self.train_ratio)*100:.0f}%):\n"
                    f"  Day 1 (22-01-2015): Train {d1_split} bins (attacks: {np.sum(d1_l[:d1_split])}) | Val {len(d1_f)-d1_split} bins (attacks: {np.sum(d1_l[d1_split:])})\n"
                    f"  Day 2 (17-02-2015): Train {d2_split} bins (attacks: {np.sum(d2_l[:d2_split])}) | Val {len(d2_f)-d2_split} bins (attacks: {np.sum(d2_l[d2_split:])})\n"
                    f"  Combined Train bins: {len(train_feats)} (attacks: {np.sum(train_labels)}, benign: {len(train_labels) - np.sum(train_labels)})\n"
                    f"  Combined Val bins:   {len(val_feats)} (attacks: {np.sum(val_labels)}, benign: {len(val_labels) - np.sum(val_labels)})"
                )
...
def run_dual_stream_pipeline(
    flow_csvs: Optional[List[Union[str, Path]]] = None,
    pcap_slices: Optional[List[Union[str, Path]]] = None,
    output_dir: Optional[Union[str, Path]] = None,
    bin_duration_sec: float = 0.5,
    sequence_length: int = 20,
    stride: int = 1,
    train_ratio: float = 0.8,
    max_packets_per_slice: int = 300_000,
) -> Dict[str, object]:
    """End-to-end multimodal execution fusing flow CSVs and packet PCAPs into 0.5s temporal bins."""
    t_start = time.time()
    logger.info("=" * 75)
    logger.info("Starting Threatora Dual-Stream Ingestion & Windowing Pipeline (CSVs + PCAPs)")
    logger.info(f"Bin Duration: {bin_duration_sec}s | Sequence Length: {sequence_length} | Stride: {stride}")
    logger.info("=" * 75)
    ...
    # 1. Flow telemetry processing (CSVs)
    target_csvs = flow_csvs if flow_csvs else resolve_default_flow_csv_paths()
    flow_feats, flow_labels, flow_mitre = preprocessor.ingest_flow_telemetry(target_csvs)

    # 2. Packet telemetry processing (PCAPs)
    target_pcaps = pcap_slices if pcap_slices else resolve_default_pcap_paths()
    packet_feats = preprocessor.ingest_packet_telemetry(
        pcap_paths=target_pcaps,
        max_packets_per_slice=max_packets_per_slice,
    )

    # 3. Modality fusion along continuous Unix epoch timeline
    features_full, labels_full, mitre_full, timestamps_full = preprocessor.fuse_modalities(
        flow_features=flow_feats,
        flow_labels=flow_labels,
        flow_mitre=flow_mitre,
        packet_features=packet_feats,
    )

    # 4. Multi-session-aware Chronological Split & Window Generation
    day1_mask = timestamps_full < 1.423e9
    day2_mask = timestamps_full >= 1.423e9

    if np.any(day1_mask) and np.any(day2_mask):
        logger.info("Multi-day experiment detected (22-01-2015 & 17-02-2015). Partitioning chronologically per capture day...")
        d1_f, d1_l, d1_m = features_full[day1_mask], labels_full[day1_mask], mitre_full[day1_mask]
        d2_f, d2_l, d2_m = features_full[day2_mask], labels_full[day2_mask], mitre_full[day2_mask]

        d1_split = int(np.floor(preprocessor.train_ratio * len(d1_f)))
        d2_split = int(np.floor(preprocessor.train_ratio * len(d2_f)))

        train_feats_all = np.concatenate([d1_f[:d1_split], d2_f[:d2_split]])
        # Fit RobustScaler strictly on train partition
        scaler = preprocessor.extractor.fit_scaler(train_feats_all)

        # Scale features using fitted scaler
        d1_tr_s = preprocessor.extractor.scale_features(d1_f[:d1_split])
        d2_tr_s = preprocessor.extractor.scale_features(d2_f[:d2_split])
        d1_val_s = preprocessor.extractor.scale_features(d1_f[d1_split:])
        d2_val_s = preprocessor.extractor.scale_features(d2_f[d2_split:])

        # Generate paired windows per capture day without seam artifacts
        s_t_tr1, s_nxt_tr1, y_tr1, yh_tr1, m_tr1 = preprocessor.generate_paired_windows(d1_tr_s, d1_l[:d1_split], d1_m[:d1_split], horizon_k=5)
        s_t_tr2, s_nxt_tr2, y_tr2, yh_tr2, m_tr2 = preprocessor.generate_paired_windows(d2_tr_s, d2_l[:d2_split], d2_m[:d2_split], horizon_k=5)

        s_t_val1, s_nxt_val1, y_val1, yh_val1, m_val1 = preprocessor.generate_paired_windows(d1_val_s, d1_l[d1_split:], d1_m[d1_split:], horizon_k=5)
        s_t_val2, s_nxt_val2, y_val2, yh_val2, m_val2 = preprocessor.generate_paired_windows(d2_val_s, d2_l[d2_split:], d2_m[d2_split:], horizon_k=5)

        s_t_train = np.concatenate([s_t_tr1, s_t_tr2])
        s_next_train = np.concatenate([s_nxt_tr1, s_nxt_tr2])
        y_train = np.concatenate([y_tr1, y_tr2])
        y_h_train = np.concatenate([yh_tr1, yh_tr2])
        m_train = np.concatenate([m_tr1, m_tr2])

        s_t_val = np.concatenate([s_t_val1, s_t_val2])
        s_next_val = np.concatenate([s_nxt_val1, s_nxt_val2])
        y_val = np.concatenate([y_val1, y_val2])
        y_h_val = np.concatenate([yh_val1, yh_val2])
        m_val = np.concatenate([m_val1, m_val2])
    else:
        # Guard-band chronological split (80% Train, 20% Val/Test)
        train_feats, train_labels, train_mitre, val_feats, val_labels, val_mitre = preprocessor.chronological_split(
            features_full, labels_full, mitre_full, timestamps=timestamps_full
        )

        # Fit RobustScaler strictly on train partition
        train_scaled, val_scaled, scaler = preprocessor.fit_and_scale(train_feats, val_feats)

        # Generate paired state transition windows + 5-horizon targets + MITRE stages
    ...
    # 7. Save Parquet datasets: train_windows_full.parquet & val_windows_full.parquet
    train_full_path = preprocessor.output_dir / "train_windows_full.parquet"
    val_full_path = preprocessor.output_dir / "val_windows_full.parquet"
    train_path = preprocessor.output_dir / "train_windows.parquet"
    val_path = preprocessor.output_dir / "val_windows.parquet"

    train_bytes = preprocessor.save_parquet(s_t_train, s_next_train, y_train, y_h_train, m_train, train_full_path)
    val_bytes = preprocessor.save_parquet(s_t_val, s_next_val, y_val, y_h_val, m_val, val_full_path)

    # Also mirror to train_windows.parquet and val_windows.parquet for downstream components
    preprocessor.save_parquet(s_t_train, s_next_train, y_train, y_h_train, m_train, train_path)
    preprocessor.save_parquet(s_t_val, s_next_val, y_val, y_h_val, m_val, val_path)

    elapsed = time.time() - t_start
    total_mb = (train_bytes + val_bytes) / (1024 * 1024)

    logger.info("=" * 75)
    logger.info("DUAL-STREAM PREPROCESSING PIPELINE EXECUTION COMPLETE")
    logger.info(f"Total Execution Time: {elapsed:.2f}s")
    logger.info(f"Timeline Span: {len(features_full)} continuous 0.5s bins")
    logger.info(f"Train Windows: {len(s_t_train)} (attacks: {np.sum(y_train)}, benign: {len(y_train) - np.sum(y_train)})")
    logger.info(f"Val Windows:   {len(s_t_val)} (attacks: {np.sum(y_val)}, benign: {len(y_val) - np.sum(y_val)})")
    logger.info(f"Tensor Shapes: S_t: {s_t_train.shape} | S_next: {s_next_train.shape} | y_horizon: {y_h_train.shape}")
    logger.info(f"Full Parquet Export: {train_full_path.name} ({train_bytes/(1024*1024):.2f}MB), {val_full_path.name} ({val_bytes/(1024*1024):.2f}MB)")
    logger.info("=" * 75)
...
def run_pipeline(
    ...
    """Execution wrapper: supports both single-CSV mode and dual-stream multimodal mode."""
    if mode == "dual":
        return run_dual_stream_pipeline(
            output_dir=output_dir,
            bin_duration_sec=bin_duration_sec,
            sequence_length=sequence_length,
            stride=stride,
...
def main():
    ...
    parser.add_argument(
        ...
        choices=["dual", "single"],
        default="dual",
        help="Preprocessing mode: 'dual' (CSVs + PCAPs) or 'single' (single CSV slice)",
    )
    parser.add_argument(
        "--input",
        "-i",
    ...
    parser.add_argument(
        "--max-packets-per-slice",
        type=int,
        default=300_000,
        help="Max packets to stream per PCAP slice (default: 300,000)",
    )
    ...
    try:
        if args.mode == "dual":
            run_dual_stream_pipeline(
                ...
                max_packets_per_slice=args.max_packets_per_slice,
            )
        else:
            run_pipeline(
