...
def train(
    ...
    default_train = repo_root / "data" / "processed" / "train_windows_full.parquet"
    if not default_train.exists():
        default_train = repo_root / "data" / "processed" / "train_windows.parquet"

    default_val = repo_root / "data" / "processed" / "val_windows_full.parquet"
    if not default_val.exists():
        default_val = repo_root / "data" / "processed" / "val_windows.parquet"

    train_path = Path(train_parquet) if train_parquet else default_train
    val_path = Path(val_parquet) if val_parquet else default_val
    ...
    # Warm-start from baseline weights if fine-tuning
    if fine_tune and best_weights_path.exists():
        try:
            logger.info(f"Loading baseline checkpoint for fine-tuning from: {best_weights_path}...")
            ckpt = torch.load(best_weights_path, map_location=device)
            state_dict = ckpt.get("model_state_dict", ckpt)
            model.load_state_dict(state_dict, strict=False)
            logger.info("Baseline weights loaded successfully for fine-tuning.")
        except Exception as exc:
            logger.warning(f"Could not load baseline checkpoint for fine-tuning: {exc}")
    ...
    logger.info("=" * 75)
    logger.info(f"Starting Training: {epochs} Epochs | Batch Size: {batch_size} | LR: {lr} | Fine-Tune: {fine_tune}")
    ...
    for epoch in range(1, epochs + 1):
        ...
        with torch.no_grad():
            for batch_s_t, batch_s_next, batch_label, batch_y_horizon, batch_mitre in val_loader:
                ...
                val_targets.append(batch_label.detach().cpu().numpy())
        ...
        try:
            val_roc_auc = float(roc_auc_score(val_targets_arr, val_preds_arr))
        except Exception:
            val_roc_auc = 0.5
        ...
        logger.info(
            ...
            f"Train Loss: {avg_train_loss:.4f} (State: {avg_train_loss_state:.4f}, Atk: {avg_train_loss_attack:.4f}) | "
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Warm-start training from baseline checkpoint if present (default: True)",
    )
    parser.add_argument(
        "--grad-clip",
        type=float,
    ...
    parser.add_argument(
        ...
        dest="fine_tune",
        action="store_false",
        help="Train model end-to-end from scratch without loading checkpoint",
    )
