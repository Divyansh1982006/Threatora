...
class CanonicalFeatureExtractor:
    ...
    def sanitize_and_extract(
        ...
        ts_candidates = ["timestamp", "stime", "ltime", "starttime", "lasttime", "time", "flow timestamp"]
        ...
        if lbl_col is not None:
            ...
            label_expr = (
                pl.when(label_col_expr.cast(pl.String).str.to_lowercase().is_in(["0", "normal", "benign", "background"]))
                .then(pl.lit(0))
        ...
        # 5. Check for mitre_stage or attack category to derive mitre_stage
        if "mitre_stage" in col_lower_map:
            exprs.append(pl.col(col_lower_map["mitre_stage"]).cast(pl.Int32, strict=False).fill_null(0).alias("mitre_stage"))
        else:
        ...
        # Select canonical columns + timestamp + label (+ mitre_stage if available)
        # (Zero data leakage: all raw identity columns are completely omitted)
    ...
    def _build_unsw_expressions(self, col_map: Dict[str, str]) -> List[pl.Expr]:
        ...
        return [
            dur_expr.cast(pl.Float32).alias("duration_norm"),
            (dbytes_expr / (sbytes_expr + 1e-6)).cast(pl.Float32).alias("byte_ratio"),
            ((spkts_expr + dpkts_expr) / (dur_expr + 1e-6)).cast(pl.Float32).alias("packet_rate"),
    ...
    def _build_cic_expressions(self, col_map: Dict[str, str]) -> List[pl.Expr]:
        ...
        return [
            dur_sec.cast(pl.Float32).alias("duration_norm"),
            (bwd_b / (fwd_b + 1e-6)).cast(pl.Float32).alias("byte_ratio"),
            rate_expr.cast(pl.Float32).alias("packet_rate"),
    ...
    def _build_ctu_expressions(self, col_map: Dict[str, str]) -> List[pl.Expr]:
        ...
        return [
            dur_expr.cast(pl.Float32).alias("duration_norm"),
            (dst_b / (src_b + 1e-6)).cast(pl.Float32).alias("byte_ratio"),
            (tot_p / (dur_expr + 1e-6)).cast(pl.Float32).alias("packet_rate"),
    ...
    def scale_features(self, features: np.ndarray) -> np.ndarray:
        ...
        if features.shape[1] < expected_feats:
            ...
            return np.clip(scaled[:, :features.shape[1]], -50.0, 50.0)
            ...
            return np.clip(np.hstack([scaled, rem]), -50.0, 50.0)

        scaled = self.scaler.transform(features).astype(np.float32)
        return np.clip(scaled, -50.0, 50.0)
