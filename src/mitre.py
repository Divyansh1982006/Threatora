...
from typing import Sequence, Dict, Any, List, Optional, Tuple, Union
...
# --------------------------------------------------------------------------
# Context-Aware Threat Stage Heuristics Mapper
# --------------------------------------------------------------------------

class ThreatStageMapper:
    """Context-aware MITRE ATT&CK kill-chain stage mapper.

    Decouples raw volumetric bursts from automatic 'Exfiltration' attribution.
    Enforces that Exfiltration (T1041 / T1048) requires BOTH anomalous byte volume/asymmetry
    AND non-standard egress behavior (elevated destination port entropy, non-standard egress ports,
    or anomalous connection patterns).
    Standard web streams (ports 80, 443, 8080, 5353, 1900, 53) with normal flag patterns
    and routine ACK transfers are classified as Benign baseline.
    """
    ...
    STANDARD_WEB_PORTS = {80, 443, 8080, 8443, 5353, 1900, 53, 67, 68, 123}

    def __init__(self, baseline_risk_threshold: float = 0.30, exfil_risk_threshold: float = 0.70):
        self.baseline_risk_threshold = baseline_risk_threshold
        self.exfil_risk_threshold = exfil_risk_threshold

    def map_stage(
        self,
        risk_score: float,
        features_window: Optional[np.ndarray] = None,
        neural_stage_idx: Optional[int] = None,
        top_ports: Optional[Union[List[int], set]] = None,
        **kwargs: Any,
    ) -> Tuple[int, str]:
        """Maps risk score and telemetry window features to MITRE stage (stage_id, stage_name).

        Returns:
            Tuple of (stage_id, stage_name).
        """
        # Baseline check: low risk is Benign
        if risk_score < self.baseline_risk_threshold:
            return BENIGN, "Benign"

        # Check raw feature window if available
        mean_rate = 0.0
        mean_syn = 0.0
        mean_ack = 0.0
        mean_rst = 0.0
        ...
        if features_window is not None:
            fw = np.atleast_2d(features_window)
            n_cols = fw.shape[1]
            if n_cols > 2:
                mean_rate = float(np.mean(fw[:, 2]))
            if n_cols > 7:
                mean_syn = float(np.mean(fw[:, 7]))
            if n_cols > 8:
                mean_ack = float(np.mean(fw[:, 8]))
            if n_cols > 12:
                mean_rst = float(np.mean(fw[:, 12]))
            if n_cols > 1:
                mean_ratio = float(np.mean(fw[:, 1]))
            if n_cols > 11:
                mean_entropy = float(np.mean(fw[:, 11]))
            if n_cols > 14:
                mean_bytes = float(np.mean(fw[:, 14]))
            elif n_cols > 0:
                mean_bytes = float(np.mean(fw[:, 0]))

        # Check if traffic exhibits standard benign web telemetry characteristics
        ports_are_standard_web = True
        if top_ports:
            ports_set = set(top_ports) if not isinstance(top_ports, set) else top_ports
            ports_are_standard_web = bool(ports_set.issubset(self.STANDARD_WEB_PORTS))

        is_routine_stream = (
            ports_are_standard_web
            and mean_syn < 0.15
            and mean_rst < 0.10
        )

        # If routine web/discovery streaming without elevated SYN/RST attacks and risk < 0.70:
        if is_routine_stream and risk_score < self.exfil_risk_threshold:
            return BENIGN, "Benign"

        # Candidate stage from neural model or heuristic
        candidate_stage = neural_stage_idx if neural_stage_idx is not None and neural_stage_idx in (1, 2, 3, 4, 5, 6) else 2

        # Context-aware Exfiltration check:
        # Exfiltration (T1041 / T1048) requires BOTH anomalous byte volume/asymmetry
        # AND non-standard egress behavior (elevated destination port entropy, anomalous payload entropy,
        # or unexpected external destination protocols / ports outside standard web ports)
        # AND risk_score >= 0.70 with verifiable protocol asymmetry or connection anomalies.
        if candidate_stage == EXFILTRATION:
            is_anomalous_egress = (not ports_are_standard_web) or (mean_entropy > 0.85 or mean_entropy < 0.05) or (mean_rst > 0.10) or (mean_syn > 0.15)
            has_volumetric_asymmetry = (mean_ratio > 3.0 or mean_bytes > 50000.0 or mean_rate > 500.0)

            if risk_score >= self.exfil_risk_threshold and is_anomalous_egress and has_volumetric_asymmetry:
                return EXFILTRATION, "Exfiltration"
            else:
                # Decouple: do not trigger Exfiltration
                if risk_score < 0.50:
                    return BENIGN, "Benign"
                elif mean_syn > 0.15:
                    return RECONNAISSANCE, "Reconnaissance"
                elif mean_rst > 0.10 or mean_entropy > 0.7:
                    return INITIAL_ACCESS, "Initial Access"
                else:
                    return COMMAND_AND_CONTROL, "Command & Control"

        stg_name = STAGE_NAMES[candidate_stage] if 0 <= candidate_stage < len(STAGE_NAMES) else "Initial Access"
        return candidate_stage, stg_name
...
def derive_stage(
    ...
    **kwargs: Any
    ...
    # 2. Exfiltration: large outbound transfer leaving enterprise on non-standard channels
    if (
        egress_ratio > 0.75
        and src_bytes > 100_000
        and frac_internal_dst < 0.3
        and n_unique_dst_ips <= 3
        and frac_c2_ports == 0.0
        and frac_lateral_ports == 0.0
    ):
        return EXFILTRATION
...
def label_flow_stage(flow_record: Dict[str, Any]) -> int:
    ...
    if dport in LATERAL_PORTS or dport in (445, 139, 3389):
        return LATERAL_MOVEMENT
    if tot_bytes > 200_000 and tot_pkts > 50 and dport not in (80, 443, 8080):
        return EXFILTRATION
