...
def is_server_live(timeout: float = 0.3) -> bool:
    try:
...
def run_all_backend_tests():
    ...
    try:
        ...
        check("model_architecture is Transformer", data.get("model_architecture") == "Threatora Temporal Transformer World Model")
        check("obs_dimension == 16",    data.get("obs_dimension") == 16)
...
def test_backend_live_server():
    import pytest
    if not is_server_live(0.3):
        pytest.skip("Threatora live backend is offline (http://127.0.0.1:5000)")
